...
map_types = ["osm", "google street", "google satellite"]

def check_presence(sentence):
    # Check if any map type is present and return the matched one
    for map_type in map_types:
        if map_type in sentence.lower():
            return True, map_type  # Return True and the found map type
    return False, None  # Return False if no map type is found
...
async def transcribe_audio(file: UploadFile = File(...)):
    try:
        ...
        # Check if any map type is present in the transcribed text
        map_found, map_type = check_presence(transcript)

        if map_found:
            # If map type is found, trigger a related function
            return {"text": transcript, "action": "Switch Map", "map_type": map_type}

        else:
            # Perform NER to find location entities
            ...
            for entity in ner_results:
                ...
                if entity['entity_group'] == 'LOC':
                    ...
                    if loc_result:
                        ...
                        print(location)  # Debugging output
                        places.append(entity['word'])
                        coords.append(location)
                else:
                    print(f"Skipping non-location entity: {entity['word']}")

            # Get the best matching action command
            best_match, similarity = get_best_match(transcript)
