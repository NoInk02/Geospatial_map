...
const handleGetDirections = async (data) => {
  ...
  return (
    <div className="relative w-screen h-screen">
      ...
      {/* Search Bar (Top Left Corner with Suggestions) */}
        ...
        <form onSubmit={handleSearch} className="flex w-full">
          ...
          >
            <svg 
              xmlns="http://www.w3.org/2000/svg" 
              viewBox="0 0 24 24" 
              className={`w-5 h-5 ${showDirectionsInput ? 'text-blue-500' : 'text-gray-500'}`}
            >
              <path 
                fillRule="evenodd" 
          ...
          {/* Directions Toggle Button (if directions exist) */}
          {directionsData && (
            <button 
              type="button" 
          ...
          {/* Clear Directions Button (if directions exist) */}
          {directionsData && (
            <button 
              type="button" 
      ...
      {/* Place Type Buttons (Top Right Corner from the 2nd code) */}
      <div className="absolute top-4 right-4 flex space-x-2 z-50">
        {placeTypes.map(type => (
          <button
            key={type}
